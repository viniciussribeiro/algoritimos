...
do {
    for (int x=1; x<=2; x++) {
    	Console.Write("Digite o número {0} (0 para fechar o programa): ", x);
    	num=int.Parse(Console.ReadLine());
	    if (num % 2 !=0) {
	    	impar++;
    } 
} Console.WriteLine("Os números ímpares digitados foram: "+impar);
} while (num!=0);
//Fim do Programa;
...
int x, not=1, soma=0, media;	
	for (x=1; x<=10; x++)	{
        ...
        Console.WriteLine("Valor inválido, favor repetir o processo com notas entre 0 e 10");
  }
}   
media= soma/10;
